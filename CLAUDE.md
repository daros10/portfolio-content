...
- `content/tech-news.json` — Feed de novedades de tecnología (`{ generatedAt, items: [{ id, title, url, source, category, publishedAt, imageUrl? }] }`). Categorías: `ai`, `cloud`, `frontend`, `mobile`, `backend`, `languages`, `tools`. **Generado automáticamente**, no mantenido a mano: `.github/workflows/tech-news.yml` corre a diario (`cron: '0 12 * * *'`, más `workflow_dispatch` para disparo manual) y ejecuta `scripts/fetch-tech-news.mjs`, que agrega ~32 feeds RSS/Atom curados (OpenAI, AWS, Google Cloud, React, Angular, Android Developers, Apple Developer, Rust Blog, etc.), filtra por antigüedad (90 días), deduplica, limita a 3 items por fuente, aplica una cuota de 8 items por categoría (para que `mobile`/`frontend`/`languages` no queden ahogadas por fuentes más prolíficas como AWS) y cap total de 48, y commitea el resultado si cambió. Si el pipeline obtiene menos de 5 items válidos, no sobrescribe el archivo (se conserva el feed del día anterior antes que publicar uno vacío/roto). `imageUrl` es opcional: se extrae primero del propio RSS (enclosure/media:thumbnail/media:content/`<img>` en el HTML) y, si el feed no trae ninguna, se intenta un fallback leyendo `og:image`/`twitter:image` de la página del artículo — solo para los items ya seleccionados, no para los cientos de items crudos. Algunas fuentes de alto volumen (AWS, OpenAI) nunca tienen imagen (AWS no publica `og:image`; OpenAI bloquea bots con 403) — es esperado, el frontend cae a un placeholder por categoría.
